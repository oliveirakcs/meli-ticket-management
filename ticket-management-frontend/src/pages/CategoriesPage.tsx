...
import { fetchSubcategories, createSubcategory, updateSubcategory, deleteSubcategory } from '../services/api';
...
const CategoriesPage: React.FC = () => {
  ...
  const handleUpdateSubcategory = async (updatedSubcategory: Subcategory) => {
    try {
      ...
      setExpandedCategoryIds(new Set([category_id]));
  ...
  const handleCreateSubcategoryClick = (category_id: string) => {
    ...
    setSelectedCategory(categories.find((category) => category.id === category_id) || null);
    setIsSubcategoryModalOpen(true);
  };
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span
                        style={{
                          ...
                          borderTop: expandedCategoryIds.has(category.id) ? 'none' : '6px solid #333',
                          borderBottom: expandedCategoryIds.has(category.id) ? '6px solid #333' : 'none',
                          transition: 'transform 0.3s ease',
                          transform: expandedCategoryIds.has(category.id) ? 'rotate(180deg)' : 'rotate(180deg)',
                          marginRight: '10px',
                        }}
                      ></span>
                      {category.name}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                        ...
                        >
                          {subcategories
                            ...
                            .map((subcategory) => (
                              <li key={subcategory.id} style={{ marginBottom: '5px', textAlign: 'center' }}>
                                {subcategory.name}
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
