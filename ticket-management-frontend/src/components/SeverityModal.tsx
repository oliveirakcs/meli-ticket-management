...
const SeverityModal: React.FC<SeverityModalProps> = ({ severity, isEditMode, onClose, onSubmit }) => {
  const [level, setLevel] = useState<number>(severity?.level || 0);
  const [description, setDescription] = useState<string>(severity?.description || '');
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', marginBottom: '5px', color: '#333' }}>Nível:</label>
            <input
              type="number"
              value={level}
          ...
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', marginBottom: '5px', color: '#333' }}>Descrição:</label>
            <input
              type="text"
              value={description}
